...
def build_research_graph():
    ...
    for node, attr in G.nodes(data=True):
        if attr.get("type") == "parameter":
            ...
            if wins > losses:
                insights.append(
                    f"🟢 Path Detected: '{node}' heavily connects to WINNING configurations ({wins} Wins vs {losses} Losses)."
                )
            elif losses > wins:
                insights.append(
                    f"🔴 Danger Node: '{node}' heavily connects to LOSING configurations ({losses} Losses vs {wins} Wins). AVOID."
                )
...
def generate_hypothesis(best_score):
    ...
    if not API_KEYS:
        print(
            "⚠️ ERROR: No API keys were loaded. Check that your file is named exactly '.env' and not '.env.txt'."
        )
        return "", ""
    ...
    while attempts < max_attempts:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={"model": OLLAMA_MODEL, "prompt": prompt, "stream": False, "temperature": 0.8},
            ...
            think_match = re.search(
                r"THINKING:\s*(.*?)(?=(HYPOTHESIS:|```|\n\nENABLE_OR_CHECKS|\*\*NEW PARAMETERS))",
                content,
            ...
            thinking = (
                ...
                if think_match
                else "No explicit THINKING block found. Using parameter mapping."
            )
...
def run_loop():
    ...
    if not all([or_checks, pivot_checks, buffer_pts, sl_pts, tp_pts, max_hold]):
        print("⚠️ LLM missed core variables. Skipping iteration.")
        time.sleep(2)
        return
    ...
    process = subprocess.Popen(
        EVAL_CMD, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8"
    )
    ...
    if score > best_score:
        ...
        print("❌ FAILED to beat high score. Reverting configuration to best known.")
