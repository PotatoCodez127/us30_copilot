                ...
                if (
                    ...
                    if direction == "LONG":
                        ...
                        for idx, candle in future_data.iloc[1:].iterrows():
                            ...
                            if mins_held >= 90:
                                if current_close < entry_price:
                                    outcome, pnl_points = (
                                        "Time Ejection ⏳",
                                        (current_close - SLIPPAGE_POINTS) - entry_price,
                                    )
                                    break
                        ...
                        for idx, candle in future_data.iloc[1:].iterrows():
                            ...
                            if mins_held >= 90:
                                if current_close > entry_price:
                                    outcome, pnl_points = "Time Ejection ⏳", entry_price - (
                                        current_close + SLIPPAGE_POINTS
                                    )
                                    break

                    color = Color.GREEN if pnl_points > 0 else Color.RED
                    print(
                        f"{color}▶ REPLAY OUTCOME: {outcome} | PnL: {round(pnl_points, 2)} pts{Color.RESET}\n"
                    )
