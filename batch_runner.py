...
def run_batch():
    session_name = input(
        f"{Color.CYAN}Enter a name for this test session (e.g., test_3, baseline): {Color.RESET}"
    ).strip()
    if not session_name:
    ...
    try:
        runs_input = input(
            f"{Color.CYAN}How many times should we run the backtest? (Default: 20): {Color.RESET}"
        ).strip()
    ...
    print(
        f"{Color.CYAN}🚀 Starting Monte Carlo Batch Engine for [{session_name}] | Target: {max_runs} Runs...{Color.RESET}"
    )
    print("Press CTRL+C at any time to stop manually.\n")
    ...
    try:
        for run_count in range(1, max_runs + 1):
            ...
            if trades_match:
                ...
                print(
                    f"{Color.YELLOW}Open the {filename} file to see the exact Python error!{Color.RESET}"
                )
                break
