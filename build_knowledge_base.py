...
def build_vector_db():
    ...
    if not os.path.exists(memory_file):
        print(f"{Color.RED}No memory bank found. Run batch_runner.py first!{Color.RESET}")
        return

    with open(memory_file, 'r') as f:
        tapes = json.load(f)

    # Initialize ChromaDB in a local folder
    ...
    for tape in tapes:
        # Prevent duplicating the same exact trade if ran multiple times
