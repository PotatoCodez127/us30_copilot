...
def build_vector_db():
    ...
    if not os.path.exists(memory_file):
        print(
            f"{Color.RED}No memory bank found at {memory_file}. Run batch_runner.py first!{Color.RESET}"
        )
        return
    ...
    if added_count > 0:
        ...
        print(
            f"{Color.GREEN}✅ Successfully vectorized and stored {added_count} unique historical setups.{Color.RESET}"
        )
    else:
