...
def analyze_setup_with_ollama(setup_payload: dict, max_retries=3) -> str:
    ...
    prompt_text += "\n\n=========================================\n"
    prompt_text += "🕸️ KNOWLEDGE GRAPH TRAVERSAL\n"
    prompt_text += "=========================================\n"
    prompt_text += f"{graph_context}\n"
    prompt_text += "Factor this historical node weight into your final confidence score."

    messages = [{"role": "user", "content": prompt_text}]

    # Grab settings from .env
    raw_model = (
        os.environ.get("OLLAMA_MODEL", "deepseek-v3.2:cloud").replace('"', "").replace("'", "")
    )
    base_url = (
        os.environ.get("OLLAMA_BASE_URL", "https://ollama.com").replace('"', "").replace("'", "")
    )
