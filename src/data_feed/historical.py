...
def fetch_chunk(symbol, start_str, end_str, api_key):
    ...
    endpoint_url = (
        f"https://api.massive.com/v2/aggs/ticker/{symbol}/range/1/minute/{start_str}/{end_str}"
    )
...
def fetch_rolling_6_months(symbol="DIA"):
    ...
    raw_candles.extend(
        fetch_chunk(symbol, (mid_date + timedelta(days=1)).strftime("%Y-%m-%d"), end_str, api_key)
    )
