...
from src.strategy.us30_ai_config_best import MAX_HOLDING_MINUTES, SL_RISK_POINTS, TP_REWARD_POINTS
    ...
    while True:
        try:
            ...
            if setup_logged_today:
                print(
                    f"[{now.strftime('%H:%M:%S')} UTC] Trade already taken today. Waiting for tomorrow...",
                    end="\r",
                )
                time.sleep(60)
                continue
            ...
            if current_day_data.empty:
                print(
                    f"[{now.strftime('%H:%M:%S')} UTC] Market closed or no data for today yet.",
                    end="\r",
                )
                time.sleep(60)
                continue
            ...
            if now.strftime("%A") == "Wednesday":
                print(
                    f"[{now.strftime('%H:%M:%S')} UTC] Mid-Week Chop Protocol: Bot sleeps on Wednesdays.",
                    end="\r",
                )
                time.sleep(60)
                continue
            ...
            if now.hour < 15:
                print(
                    f"[{now.strftime('%H:%M:%S')} UTC] OR Formed: {or_high:.2f} / {or_low:.2f}. Waiting for 15:00 UTC Sniper Window...",
                    end="\r",
                )
                time.sleep(60)
                continue
            ...
            if now.hour > 15 or (now.hour == 15 and now.minute > 30):
                print(
                    f"[{now.strftime('%H:%M:%S')} UTC] Golden Window Closed. No valid momentum today.",
                    end="\r",
                )
                time.sleep(60)
                continue
            ...
            print(
                f"[{now.strftime('%H:%M:%S')} UTC] SNIPER ACTIVE | Price: {candle_1m['close']:.2f} | Scanning tick-by-tick...",
                end="\r",
            )
            ...
            if payload:
                ...
                is_bull_trap = (
                    "Opening Range Low" in payload["trigger"] and raw_entry > central_pivot
                )
                is_bear_trap = (
                    "Opening Range High" in payload["trigger"] and raw_entry < central_pivot
                )
                ...
                if not is_bull_trap and not is_bear_trap:
                    ...
                    try:
                        ...
                        if os.path.exists(db_path):
                            ...
                            if rag_collection.count() > 0:
                                print(
                                    f"{Color.CYAN}🧠 Querying RAG Memory Bank for similar live setups...{Color.RESET}"
                                )
                    ...
                    if (
                        dir_match
                        and dir_match.group(1).upper() in ["LONG", "SHORT"]
                        and sl_match
                        and tp_match
                    ):
                        ...
                        print(
                            f"Time Limit:  Eject trade after exactly {MAX_HOLDING_MINUTES} minutes! ⏳"
                        )  # <-- NEW LINE
                        print("\a")
