...
def aggregate():
    session_name = input(
        f"{Color.CYAN}Enter the name of the test session to aggregate (e.g., test_3): {Color.RESET}"
    ).strip()
    if not session_name:
    ...
    if not os.path.exists(output_dir):
        print(
            f"{Color.RED}No folder found at {output_dir}. Run batch_runner.py first!{Color.RESET}"
        )
        return
