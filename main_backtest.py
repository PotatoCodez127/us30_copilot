...
from src.strategy.us30_ai_config import MAX_HOLDING_MINUTES, SL_RISK_POINTS, TP_REWARD_POINTS
...
def simulate_trade(
    direction: str, raw_entry: float, future_data: pd.DataFrame, trigger_time: pd.Timestamp
):
        ...
        if daily_setups:
            for setup in daily_setups:
                ...
                if (
                    ...
                    future_data = current_day_data.loc[
                        setup["timestamp"] : f"{current_date_str} 20:00:00+00:00"
                    ]
                    ...
                    setup["holding_time_mins"] = round(
                        (exit_time - trigger_time).total_seconds() / 60.0, 1
                    )

                    color = Color.GREEN if pnl_points > 0 else Color.RED
                    print(
                        f"{color}▶ EXECUTED {direction}: {outcome} | PnL: {setup['pnl_points']} pts{Color.RESET}"
                    )
                ...
                if os.path.exists(memory_file):
                    with open(memory_file, "r", encoding="utf-8") as f:
                        try:
                            ...
                            print(
                                f"{Color.YELLOW}⚠️ Warning: memory bank was corrupted/empty. Overwriting with new data.{Color.RESET}"
                            )
                            existing_memory = []
...
if __name__ == "__main__":
    ...
    if os.path.exists(rolling_file):
        ...
        print(
            f"{Color.RED}⚠️ No rolling data found. Please run rolling_researcher.py to fetch data first.{Color.RESET}"
        )